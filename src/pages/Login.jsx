...
const Login = () => {
  ...
  const { navigate, supabaseSignIn, supabaseSignUp, setUserProfile, adminLogin } = useContext(ShopContext);
  ...
  const onSumbitHandler = async (event) => {
    ...
    try {
      if (currentState === 'Sign up') {
        // Requirement 2: Signup creates account, redirects user to Login page view, and does NOT auto-login.
        const result = await supabaseSignUp(formattedName, cleanEmail, password);
        if (result.success) {
        ...
        // Requirement 3 & 7: Login using Supabase Auth, navigate admin@gmail.com -> /admin, others -> /
        await supabaseSignIn(cleanEmail, password);
      }
    } catch (error) {
